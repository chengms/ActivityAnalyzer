...
# 组合使用（强制清理 + 指定版本号）
.\pack-with-mirror.ps1 dist:win:portable --force --version 1.0.2
...
**📌 版本号参数：**
- 使用 `--version <version>` 或 `-v <version>` 可以在打包时自动更新 `package.json` 中的版本号
- 版本号格式建议使用语义化版本号（如：`1.0.1`、`1.2.3`）
- 脚本会在构建前更新版本号，并在打包完成后显示最终使用的版本号
...
- [ ] 更新版本号（可在打包时使用 `--version` 参数自动更新）
- [ ] 运行 `npm run build` 确保构建成功
- [ ] 测试构建后的应用 (`npm start`)
- [ ] 准备图标文件 (`build/icon.ico`)
- [ ] 运行打包命令（推荐使用 `.\pack-with-mirror.ps1 dist:win:portable --version <版本号>`）
- [ ] 测试安装程序
- [ ] 测试卸载程序
- [ ] 检查文件大小是否合理
