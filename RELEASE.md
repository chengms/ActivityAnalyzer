...
```powershell
# 使用打包脚本打包（推荐：自动更新版本号）
.\pack-with-mirror.ps1 dist:win:portable --version 1.0.0
# 或使用简写
.\pack-with-mirror.ps1 dist:win:portable -v 1.0.0

# 如果不指定版本号，使用 package.json 中的当前版本号
.\pack-with-mirror.ps1 dist:win:portable
```

打包完成后，可执行文件位于 `release/活动分析器-1.0.0.exe`

**提示：** 使用 `--version` 参数可以在打包时自动更新 `package.json` 中的版本号，无需手动编辑。
...
```powershell
# 使用打包脚本直接指定版本号（推荐，更灵活）
.\pack-with-mirror.ps1 dist:win:portable --version 1.0.0
# 或使用简写
.\pack-with-mirror.ps1 dist:win:portable -v 1.0.0

# 然后手动完成 Git 操作和 GitHub Release 创建
```
